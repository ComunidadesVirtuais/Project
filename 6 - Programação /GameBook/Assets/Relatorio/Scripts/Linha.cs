...
public class Linha : MonoBehaviour {
	...
	void Start () {
		...
		for (int j=1; j<=5; j++) {
			...
			int k=1;
			float media=0,cont=0,mediaBest=0;
						while (PlayerPrefs.HasKey("APMG"+j+"N"+k)) {

				print("Last Point " + j +" " +PlayerPrefs.GetInt("LPMG"+j+"N"+k));
				print("Best Point " + j +" " + PlayerPrefs.GetInt("BPMG"+j+"N"+k));
				print("AVG Point "  + PlayerPrefs.GetFloat("APMG"+j+"N"+k));
				print("Turn "       + PlayerPrefs.GetInt("TMG"+j+"N"+k));
							
							mediaBest = mediaBest + PlayerPrefs.GetInt("BPMG"+j+"N"+k);
							media = (media*cont + (PlayerPrefs.GetFloat("APMG"+j+"N"+k)*PlayerPrefs.GetInt("TMG"+j+"N"+k)))/(cont+PlayerPrefs.GetInt("TMG"+j+"N"+k));
							cont = cont + PlayerPrefs.GetInt("TMG"+j+"N"+k);
							k++;
						}		
						print("media "+media+" cont "+cont+" media best"+mediaBest);
						if (media == 0)
								valores [j-1] = 0;
						else
								valores [j-1] = 3 * media/ (mediaBest/(k-1));
				}
