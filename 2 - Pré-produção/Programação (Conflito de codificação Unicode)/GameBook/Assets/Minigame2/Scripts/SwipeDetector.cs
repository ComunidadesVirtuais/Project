...
//					float dist = Mathf.Sqrt(Mathf.Pow(delta.x, 2) + Mathf.Pow (delta.y, 2));
	...
	{
		if(Input.GetKeyUp(KeyCode.RightArrow)) this.GetComponent<Minigame>().PlayerAnswer(2);
		else if(Input.GetKeyUp(KeyCode.LeftArrow)) this.GetComponent<Minigame>().PlayerAnswer(1);
		else if(Input.GetKeyUp(KeyCode.UpArrow)) this.GetComponent<Minigame>().PlayerAnswer(3);
		else if(Input.GetKeyUp(KeyCode.DownArrow)) this.GetComponent<Minigame>().PlayerAnswer(4);
	}
	...
	private bool SwipeVertical(Vector2 touchPosition, float angle)
	{
			...
			
		{
			...
			if (swipeValue > 0)//up swipe
			{
				...
				{
					willSwipe = true;
					this.GetComponent<Minigame>().PlayerAnswer(3);
					//main.PlayerAnswer(3);
				}
			}
			else if (swipeValue < 0)//down swipe
			{
				...
				{
					willSwipe = true;
					this.GetComponent<Minigame>().PlayerAnswer(4);
					//main.PlayerAnswer(4);
				}
			}
		}
	...
	private void SwipeHorizontal(Vector2 touchPosition, float angle)
	{
			...
			
		{
			...
			if (swipeValue > 0)//right swipe
			{
				...
				{
					this.GetComponent<Minigame>().PlayerAnswer(2);
					//main.PlayerAnswer(2);
				}
			}
			else if (swipeValue < 0)//left swipe
			{
				if(((touchPosition.y > startPos.y) && (angle >= -anguloMargemErro)) || ((touchPosition.y < startPos.y) && (angle <= anguloMargemErro)) || (touchPosition.y == startPos.y))
				{
					this.GetComponent<Minigame>().PlayerAnswer(1);
					//main.PlayerAnswer(1);
				}
			}
		}
