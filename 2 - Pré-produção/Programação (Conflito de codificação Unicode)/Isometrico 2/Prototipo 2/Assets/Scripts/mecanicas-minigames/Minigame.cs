...
public class Minigame : MonoBehaviour {
	...
	private void EndGame()
	{
		...
		else
		{
			...
			this.transform.parent.GetComponent<PopupMecanica3>().FinalizarJogo();
		}
	}
	...
	private void PlaceAtRandomPosition()
	{
		...
		arrows.transform.localPosition = randomPos;
	...
	private Vector3 GetRandomPositionScreen(float size)
	{
		...
		/*Vector3 max = new Vector3(this.transform.parent.renderer.bounds.size.x, this.transform.parent.renderer.bounds.size.y, 0);
		Vector3 min = new Vector3(0, 0, 0);
		
		Vector3 worldPosMax = Camera.main.ScreenToWorldPoint(max);
		Vector3 worldPosMin = Camera.main.ScreenToWorldPoint(min);*/

		float distanciaEntreSetas = Mathf.Abs(GameObject.Find ("Arrow3").transform.position.y - GameObject.Find ("Arrow1").transform.position.y); // a seta que fica mais em baixo
		Vector3 worldPosMax = new Vector3(this.transform.parent.renderer.bounds.size.x/2 - size/2, this.transform.parent.renderer.bounds.size.y/2 - size/2, 0);
		Vector3 worldPosMin = new Vector3(0 - this.transform.parent.renderer.bounds.size.x/2 + size/2, 0 - this.transform.parent.renderer.bounds.size.y/2 + distanciaEntreSetas, 0);
		Vector3 randomPoint = new Vector3 (Random.Range (worldPosMin.x, worldPosMax.x), Random.Range (worldPosMin.y, worldPosMax.y), 0);

		print ("Tamanho do popup: " + this.transform.parent.renderer.bounds.size + " modificad: " + worldPosMin + "x" + worldPosMax + " random point: " + randomPoint);
	...
	private bool isPositionValid(Vector3 random, float size)
	{
		...
		//Vector3 limite = Camera.main.ScreenToWorldPoint (v);

		Vector3 limite = new Vector3(this.transform.parent.renderer.bounds.size.x/2, this.transform.parent.renderer.bounds.size.y/2, 0);
