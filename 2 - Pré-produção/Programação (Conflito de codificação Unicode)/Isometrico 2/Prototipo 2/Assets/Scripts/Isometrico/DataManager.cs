...
public class DataManager
{
	...
	public static int qtdDinheiro;
	public static string[] coinsNames = null;
	public static int cenaMoedas;
	...
	public static void SaveAll(List<Dictionary<string,string>> variaveis)
	{
		...
		save += "Vidas:" + GameObject.Find ("Vidas").GetComponent<Life> ().GetQtdVidas () + ";";

		save += "Dinheiro:" + GameObject.Find ("QtdDinheiro").GetComponent<DinheiroInterface> ().GetQuantidadeDinheiro() + ";";
	...
	public static void LoadSave()
	{
		...
		if (PlayerPrefs.HasKey ("Save"))
		{
			...
			for(int i =0; i < partesSave.Length; i++)
			{
				...
				switch(tipos[0])
				{
					...
					case "Vidas":
					{
						qtdVidas = int.Parse(tipos[1]);
						break;
					}
					case "Dinheiro":
					{
						qtdDinheiro = int.Parse(tipos[1]);
						break;
					}
					case "Posicao_Player":
					{
		...
		if (PlayerPrefs.HasKey ("MoedasColetadas"))
		{
			...
			partesSave = save.Split(':');
			cenaMoedas = int.Parse (partesSave[0]);
			coinsNames = partesSave[1].Split(',');
		}
	}
